...
test("generateLifeEventPlaybook falls back when AI payload breaks tone rules", async () => {
  const result = await generateLifeEventPlaybook(
    {
      eventType: "buying_apartment",
      title: "Buying our first apartment",
      ...
      jsonGenerator: async () =>
        JSON.stringify({
          actions: Array.from({ length: 5 }, () => ({
            ...
            description:
              "Sell the concentrated equity funds immediately and move into safer bonds.",
            category: "financial",
            priority: "high",
