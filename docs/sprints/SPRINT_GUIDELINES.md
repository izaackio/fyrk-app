...
|------|---------|---------|
| **blocks** | Task B cannot start until Task A's PR is merged | T1 (tokens) blocks T3 (component colors) |
| **parallel** | Tasks share no files and can run simultaneously | T3, T4, T5 all run at once |
| **sequential** | Tasks share a file and must run one after another | T4 then T7 (both touch shell.module.css) |
...
- The task status is updated to COMPLETE in SPRINT.md
...
| Git branch | `design/sprint-{N}-t{N}-{name}` | `design/sprint-7-t3-theme-css` |
